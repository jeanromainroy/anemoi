...
// Check inputs
if(!isset($data->vac_pc) && !isset($data->vac_vc) && !isset($data->cpap) && !isset($data->bipap)){

    // set response code
    http_response_code(400);
 
    // display message: unable to create user
    echo json_encode(array("message" => "Unable to create session. Missing Inputs"));
...
if(
	...
	// tell the user
	echo json_encode(array("message" => "Session (". $data->id . ") was created."));
}
...
// if unable to create the branch, tell the user
	...
	// tell the user
	echo json_encode(array("message" => "Error : Unable to create session."));
}
?>